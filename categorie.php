...
<header>
    ...
    <li><span><a href="pageMenu.php"><img name="id" class="button" height="40px" width="40px" src="Images/588a64e0d06f6719692a2d10.png"></a><span> </li>
...
}elseif (isset($_POST['plats'])){
                        echo'<h2>categorie:Plats</h2>';
                    }elseif(isset($_POST['dessert'])){
                            echo'<h2>categorie:dessert</h2>';
                        }elseif(isset($_POST['entre'])){
                            echo'<h2>categorie:entre</h2>';
                        }
...
<ul>
    ...
    <?php
     ...
     if (isset($_POST['plats'])){
       ...
       $table = 'type';
        ...
        $result1 = $bdd->query("SELECT * FROM $table WHERE type2='plat'") or die($bdd->error);
        $data2 = $result1->fetchAll() ;
       
        if (!isset($_POST['plats'])){
           require_once 'phpfunction/display (1).php';
       }else{
        for($i=0;$i<count($data2);$i++){
            ...
            <div class="block">
                    ...
                    <a href="type.php?type='.$data2[$i]['type'].'"><img src="'.$data2[$i]['img_dir'].'" width="100%" height="100%"></a>
                  </div>
                  
                  <h3 class="title-Menu">'.$data2[$i]['name'].'</h3>
                </li>
  ...
  (isset($_POST['dessert'])){
    ...
    $table = 'type';
     ...
     $result1 = $bdd->query("SELECT * FROM $table WHERE type2='dessert'") or die($bdd->error);
     $data2 = $result1->fetchAll() ;
...
(isset($_POST['entre'])){
    ...
    $table = 'type';
     ...
     $result1 = $bdd->query("SELECT * FROM $table WHERE type2='entre'") or die($bdd->error);
     $data2 = $result1->fetchAll() ;
